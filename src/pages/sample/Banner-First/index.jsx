...
import BannerFirstTable from './BannerFirstTable';
...
const Index = () => {
  ...
  const {
    ...
    isLoading: getCategoryLoading,
      refetch
  } = useQuery('banner-first-get', () => apiService.getData('/about/header-banner-first'), {
    // enabled:false,
    onError: (error) => {
  ...
  const serachProduct = (value) => {
    ...
    const filterData = data?.filter(
      (data) =>
        data.brand.toLowerCase().includes(value.toLowerCase()) ||
        data.season.toLowerCase().includes(value.toLowerCase()),
    );
